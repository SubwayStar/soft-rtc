...
def eval(
    ...
    def execute_chunk(carry, _):
        def step(carry, action):
            ...
            return (rng, next_obs, next_env_state), (done, env_state, info, action)
        ...
        (rng, next_obs, next_env_state), (dones, env_states, infos, actions) = jax.lax.scan(
        ...
        return (rng, next_obs, next_env_state, next_action_chunk, next_n), (dones, env_states, infos, actions)
    ...
    _, (dones, env_states, infos, actions) = jax.lax.scan(
        execute_chunk,
        (rng, obs, env_state, action_chunk, n),
        None,
        length=scan_length,
    )
    dones, env_states, infos, actions = jax.tree.map(
        lambda x: x.reshape(-1, *x.shape[2:]),
        (dones, env_states, infos, actions),
    )
    assert dones.shape[0] >= env_params.max_timesteps, f"{dones.shape=}"
    return_info = {}
    time_idxs = jnp.arange(dones.shape[0])[:, None]
    first_done_idx = jnp.argmax(dones, axis=0)
    valid_mask = time_idxs <= first_done_idx[None, :]
    ...
    action_delta = jnp.linalg.norm(actions[1:] - actions[:-1], axis=-1)
    action_delta_mask = valid_mask[1:] & valid_mask[:-1]
    return_info["action_delta_l2"] = jnp.sum(action_delta * action_delta_mask) / (
        jnp.sum(action_delta_mask) + 1e-8
    )
    action_jerk = jnp.linalg.norm(actions[2:] - 2 * actions[1:-1] + actions[:-2], axis=-1)
    action_jerk_mask = valid_mask[2:] & valid_mask[1:-1] & valid_mask[:-2]
    return_info["action_jerk_l2"] = jnp.sum(action_jerk * action_jerk_mask) / (
        jnp.sum(action_jerk_mask) + 1e-8
    )
    for key in ["match"]:
        if key in infos:
