...
class Config:
    ...
    soft_prefix: str = "paperd2-soft"
    soft_step: int = 0
    experiment_name: str = "soft_variant"
    soft_delay: int = 4
    ...
    seed: int = 0
    output_csv: str = "eval_output/soft_variant.csv"
...
def main(config: Config):
    ...
    completed_keys = {(row["level"], int(row["delay"])) for row in rows}
    ...
    for level_idx, level_path in enumerate(config.level_paths):
        ...
        for delay in config.delays:
            ...
            eval_config = eval_flow.EvalConfig(
                ...
                method=eval_flow.RealtimeMethodConfig(),
                model=soft_model_config,
            )
