...
PrefixAttentionEndMode: TypeAlias = Literal["scaled", "fixed", "offset"]
...
class ModelConfig:
    ...
    simulated_prefix_attention_horizon: int | None = None
    simulated_prefix_attention_end_mode: PrefixAttentionEndMode = "scaled"
    simulated_prefix_attention_delay_multiplier: float = 2.0
    simulated_prefix_attention_offset: int = 0
    simulated_prefix_attention_schedule: PrefixAttentionSchedule = "zeros"
...
def get_prefix_attention_end(
    start: int,
    max_end: int,
    end_mode: PrefixAttentionEndMode,
    delay_multiplier: float,
    offset: int,
) -> jax.Array:
    """Returns an effective overlap end that scales with delay.

    `scaled`: `min(ceil(delay_multiplier * start), max_end)`
    `fixed`: `max_end`
    `offset`: `min(start + offset, max_end)`

    For `scaled` and `offset`, `start == 0` returns 0, so soft conditioning exactly
    recovers the no-RTC case at zero delay. The `fixed` mode intentionally keeps a
    delay-independent window for ablations.
    """
    start = jnp.asarray(start)
    max_end = jnp.asarray(max_end)
    if end_mode == "fixed":
        return max_end
    if end_mode == "scaled":
        scaled_end = jnp.ceil(start * delay_multiplier).astype(jnp.int32)
        return jnp.where(start == 0, 0, jnp.minimum(max_end, jnp.maximum(start, scaled_end)))
    if end_mode == "offset":
        offset_end = start + offset
        return jnp.where(start == 0, 0, jnp.minimum(max_end, jnp.maximum(start, offset_end)))
    raise ValueError(f"Invalid end mode: {end_mode}")
...
class FlowPolicy(nnx.Module):
    def __init__(
        ...
        *,
        ...
        self.simulated_prefix_attention_horizon = config.simulated_prefix_attention_horizon
        self.simulated_prefix_attention_end_mode = config.simulated_prefix_attention_end_mode
        self.simulated_prefix_attention_delay_multiplier = config.simulated_prefix_attention_delay_multiplier
        self.simulated_prefix_attention_offset = config.simulated_prefix_attention_offset
        self.simulated_prefix_attention_schedule = config.simulated_prefix_attention_schedule
    ...
    def realtime_action(
        ...
        def step(carry, _):
            ...
            if self.simulated_delay is not None:
                simulated_prefix_attention_horizon = (
                    self.simulated_prefix_attention_horizon
                    if self.simulated_prefix_attention_horizon is not None
                    else inference_delay
                )
                simulated_prefix_attention_end = get_prefix_attention_end(
                    inference_delay,
                    simulated_prefix_attention_horizon,
                    self.simulated_prefix_attention_end_mode,
                    self.simulated_prefix_attention_delay_multiplier,
                    self.simulated_prefix_attention_offset,
                )
                weights = get_prefix_weights(
                    inference_delay,
                    simulated_prefix_attention_end,
                    self.action_chunk_size,
                    self.simulated_prefix_attention_schedule,
                )
                x_t = weights[None, :, None] * prev_action_chunk + (1 - weights)[None, :, None] * x_t
                time_chunk = weights[None, :] + (1 - weights)[None, :] * time
    ...
    def loss(self, rng: jax.Array, obs: jax.Array, action: jax.Array):
        ...
        prefix_attention_horizon = (
            self.simulated_prefix_attention_horizon
            if self.simulated_prefix_attention_horizon is not None
            else self.simulated_delay
        )
        weights = jax.vmap(
            lambda sampled_delay: get_prefix_weights(
                sampled_delay,
                get_prefix_attention_end(
                    sampled_delay,
                    prefix_attention_horizon,
                    self.simulated_prefix_attention_end_mode,
                    self.simulated_prefix_attention_delay_multiplier,
        ...
        time = weights + (1 - weights) * time[:, None]
        x_t = (1 - time[:, :, None]) * noise + time[:, :, None] * action
        pred = self(obs, x_t, time)
        loss = jnp.square(pred - u_t)
        loss_weights = (1 - weights)[:, :, None]
        return jnp.sum(loss * loss_weights) / (jnp.sum(loss_weights) + 1e-8)
